...
export default function Anxiety() {
  ...
  //     category,
  //     products.filter(product => product.title=== 'GoStress' || 'GoSleep')
  //   ])
  // );
  ...
  const filteredProducts = Object.fromEntries(
    Object.entries(productsData).map(([category, products]) => [
      category,
      products.filter(product => category === 'Best Seller' && (product.title === 'GoStress' || product.title === 'GoSleep'))
    ])
  );
  
  // Remove empty categories (if needed)
  const nonEmptyFilteredProducts = Object.fromEntries(
    Object.entries(filteredProducts).filter(([_, products]) => products.length > 0)
  );
  
  // console.log(nonEmptyFilteredProducts);
  ...
  // const filtered = productsData.fil
  ...
  return (
    ...
    //   <Breadcrumb />
    //   <h2 className="text-3xl font-bold mb-8 px-4">Anxiety & Overthinking</h2>
...
<div className="max-w-screen-xl mx-auto">
  ...
  {/* Product Cards Section */}
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-4">
    {Object.entries(nonEmptyFilteredProducts).map(([products]) =>
      products.map((product) => (
        ...
        >
          ...
          <p className="mt-1 font-bold pb-2 text-[#465f33]">
            FOR: <span className="text-black">{product.forText}</span>
          </p>
          <p className="mt-1 font-bold pb-8 text-[#465f33]">
            WITH: <span className="text-black">{product.withText}</span>
          </p>
          <button className="mt-4 py-2 px-4 bg-[#465f33] hover:bg-[#294713] text-white rounded-lg w-full">
            Add to Cart
          </button>
